 ...
 */
(function($) { $.fn.fontSizer = function(options) { var defaults = { 'target' : 'body', 'type' : 'em', 'default' : 0.8, 'step' : 0.1, 'max' : 5.0, 'min' : 0.1, 'cookie' : false, 'expire' : 30, 'id_increase': 'fs_increase', 'id_decrease': 'fs_decrease', 'id_reset' : 'fs_reset', }; var setting = $.extend(defaults, options); var tags = new Array('div','td','tr'); var cookie_name = 'fontSizer'; var Default = setting.default; var fontChange = function(value) { if ( ! document.getElementById ) return; var fontSize = parseFloat(Default), value; if ( value != 0 ) { fontSize += value * setting.step; if (fontSize < setting.min) { fontSize = setting.min;} else if (fontSize > setting.max) { fontSize = setting.max;}
if ( setting.cookie == true ) { createCookie(cookie_name, fontSize, setting.expire);}
} else { fontSize = setting.default; if ( setting.cookie == true ) { eraseCookie(cookie_name);}
}
